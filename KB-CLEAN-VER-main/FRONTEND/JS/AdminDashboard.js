(function () {
	...
	function renderDashboardStats() {
		...
		const cards = state.role === 'ngo'
			? [
					{ label: 'Active Campaigns', value: '3', sub: '2 ongoing', color: 'sky', icon: 'M12 21s-8-4.5-8-11V5l8-3 8 3v5c0 6.5-8 11-8 11z' },
					{ label: 'Total Raised', value: fmtMoney(168000), sub: '+12% this month', color: 'green', icon: 'M12 1v22M17 6H9a4 4 0 0 0 0 8h6a4 4 0 1 1 0 8H7' },
					{ label: 'Donors Reached', value: '431', sub: 'Across all drives', color: 'gold', icon: 'M17 21v-2a4 4 0 0 0-4-4H5' },
					{ label: 'Pending Tasks', value: '4', sub: 'Need action today', color: 'red', icon: 'M12 8v4l3 3M22 12A10 10 0 1 1 12 2' }
				]
			: [
					{ label: 'Total Donations', value: fmtMoney(1124000), sub: '+18% this month', color: 'green', icon: 'M12 1v22M17 6H9a4 4 0 0 0 0 8h6a4 4 0 1 1 0 8H7' },
					{ label: 'Active Campaigns', value: '14', sub: '3 pending approvals', color: 'sky', icon: 'M12 21s-8-4.5-8-11V5l8-3 8 3v5c0 6.5-8 11-8 11z' },
					{ label: 'NGO Partners', value: '9', sub: '7 verified', color: 'gold', icon: 'M4 21h16M7 21V8h10v13' },
					{ label: 'Open Flags', value: '2', sub: 'Moderation needed', color: 'red', icon: 'M12 9v4M12 17h.01M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z' }
				];
	...
	function renderActivityFeed() {
		...
		const feed = [
			{ title: 'Campaign approved: Tondo Learning Kits', meta: 'by Super Admin', time: '5m', color: 'green' },
			{ title: 'New NGO registration submitted', meta: 'Hope in Tondo', time: '13m', color: 'sky' },
			{ title: 'Flag raised on Mobile Clinic Upgrades', meta: 'Missing liquidation file', time: '40m', color: 'red' },
			{ title: 'Monthly report generated', meta: 'Analytics module', time: '1h', color: 'gold' }
		];
	...
	function renderCharts() {
		...
		state.charts.daily = createChart('dailyChart', {
			...
			data: {
				...
				datasets: [{ label: 'PHP', data: [178000, 214000, 163000, 241000], backgroundColor: '#4a9cc7' }]
			},
			options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } }
		});
		...
		state.charts.category = createChart('categoryChart', {
			type: 'pie',
			data: {
				labels: ['Education', 'Health', 'Disaster', 'Community'],
				datasets: [{ data: [40, 28, 20, 12], backgroundColor: ['#4a9cc7', '#2e9e6e', '#d4821a', '#7c4de8'] }]
			},
			options: { responsive: true, maintainAspectRatio: false }
		});
		...
		state.charts.donor = createChart('donorChart', {
			...
			data: {
				...
				datasets: [{ label: 'New donors', data: [42, 55, 61, 73, 89], borderColor: '#c9a84c', tension: 0.35 }]
			},
			options: { responsive: true, maintainAspectRatio: false }
		});
	}
	...
	function saveCampaign() {
		const campaign = createCampaignRecord('draft');
		if (!campaign) return;

		const campaigns = readCreatedCampaigns();
		campaigns.unshift(campaign);
		writeCreatedCampaigns(campaigns);

		showToast('Campaign saved as draft with payout details.', 'success');
		closeModal('createCampaignModal');
	}

	function submitCampaign() {
		const campaign = createCampaignRecord('pending');
		if (!campaign) return;
		...
		showToast('Campaign submitted. Donors can now view your payout details.', 'success');
		closeModal('createCampaignModal');
	}

	function openCampaignDetail(id) {
		const campaign = data.campaigns.find((c) => c.id === Number(id));
		if (!campaign) return;

		if (qs('cdTitle')) qs('cdTitle').textContent = campaign.title;
		if (qs('cdBody')) {
			qs('cdBody').innerHTML = `
				<p style="color:var(--text-mid);margin-bottom:14px">Category: <strong>${campaign.category}</strong></p>
				<p style="color:var(--text-mid);margin-bottom:8px">Raised: ${fmtMoney(campaign.raised)} of ${fmtMoney(campaign.goal)}</p>
				<p style="color:var(--text-mid)">Current status: <span class="badge ${badgeClass(campaign.status)}">${campaign.status}</span></p>
			`;
		}
		if (qs('cdFooter')) {
	...
	function openApprovalModal(mode, index) {
		...
		if (qs('approvalTitle')) qs('approvalTitle').textContent = mode === 'approve' ? 'Approve Campaign' : 'Reject Campaign';
		if (qs('approvalDesc')) qs('approvalDesc').textContent = `${mode === 'approve' ? 'Approve' : 'Reject'} ${row.campaign} from ${row.ngo}?`;
		if (qs('approvalFooter')) {
			qs('approvalFooter').innerHTML = `
				...
				<button class="btn ${mode === 'approve' ? 'btn-success' : 'btn-danger'} btn-sm" onclick="submitApproval('${mode}', ${index})">Confirm</button>
			`;
		}
	...
	function submitApproval(mode, index) {
		const row = data.approvals[index];
		if (!row) return;
